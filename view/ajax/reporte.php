<?php
  ...
  $db = new Conexion();
	//Consulta
	$tick = $db->query("SELECT * FROM tblp_ticket WHERE Encargado = '$encargado' AND FecCap LIKE '$Ids-%' ORDER BY Estatus");
  ...
  $consulta =0;
	$gdImage = imagecreatefrompng('../../assets/logo/logo.png');//Logotipo
    ...
    while($x = $db->recorrer($sql)){
      ...
      while($y = $db->recorrer($sql1)){
        ...
        while($w = $db->recorrer($sql2)){
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('J'.$fila, $w['Nombre']." ".$w['APaterno']." ".$w['AMaterno']);
